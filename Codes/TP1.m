function [U_h_plein,Nbtri,Tri_ar,Na,Numaretes,Numtri,Coorneu,Ns,Refneu,Barycentres,Aire_tri,Ar_tri] = TP1(nom_maillage)
...
for F=1:Na
    if Numaretes(F,3) == 1
        s = 0;
        for K=1:2
            if Ar_tri(F,K)~=0 %Inutile, Numaretes(F,3)=1 donc il y a deux triangles
                x = (Coorneu(Numaretes(F,1),1) + Coorneu(Numaretes(F,2),1))/2; %sortir de la seconde boucle
